...
export function RegisterForm({ onSuccess, onSwitchToLogin }: RegisterFormProps) {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      await useCases.registerUser.execute({ email, password, name });
      onSuccess?.();
    } catch (err) {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      if (!useCases.loginWithGoogle) {
        setMessage('Login com Google disponível apenas com Firebase ativado.');
        return;
      }
      await useCases.loginWithGoogle.execute();
      onSuccess?.();
    } catch (err: any) {
      setMessage(err.message || 'Não conseguimos concluir agora. Você pode tentar mais tarde.');
    } finally {
      setLoading(false);
    }
  }
        ...
        <>
          <div className="relative">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-dark-bg-primary text-dark-text-secondary">ou</span>
            </div>
          </div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-gray-50 text-gray-700 py-3 px-4 rounded-lg border border-gray-300 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm font-normal"
          >
